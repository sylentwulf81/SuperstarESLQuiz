...
export const DiceRoller: React.FC<DiceRollerProps> = ({
  ...
  const renderPips = (num: number) => {
    ...
    return (
      <div className={`grid grid-cols-3 grid-rows-3 pointer-events-none ${
        compact ? 'w-12 h-12 p-1.5' : 'w-16 h-16 sm:w-20 sm:h-20 p-2.5 sm:p-3'
      }`}>
              ...
              {isPip && (
                <div
                  className={`rounded-full shadow-inner ${
                    num === 1 && isCenter
                      ? compact
                        ? 'w-3 h-3 bg-red-600 shadow-[0_0_8px_rgba(220,38,38,0.8)]'
                        : 'w-4 h-4 sm:w-5 sm:h-5 bg-red-600 shadow-[0_0_8px_rgba(220,38,38,0.8)]'
                      : compact
                        ? 'w-2 h-2 bg-slate-900 shadow-[inset_0_1px_2px_rgba(0,0,0,0.6)]'
  ...
  return (
    <div className={`rounded-3xl border ${bgStyles} flex flex-col items-center text-center ${
      compact ? 'p-2.5 gap-2 w-full' : 'p-4 sm:p-5 gap-3.5'
    }`}>
      <div>
        <h4 className={`font-mario text-white text-shadow-mario flex items-center justify-center gap-2 ${
          compact ? 'text-base sm:text-lg' : 'text-xl sm:text-2xl'
        }`}>
          <Dices className={`${compact ? 'w-5 h-5' : 'w-6 h-6'} text-yellow-300`} />
          {title}
        </h4>
        {!compact && (
      ...
      {/* 3D Mario Die */}
      <div className={`relative ${compact ? 'my-0.5' : 'my-2'}`}>
        <motion.button
          type="button"
          onClick={startRoll}
          ...
          className={`${compact ? 'w-20 h-20 rounded-2xl border-[3px]' : 'w-28 h-28 sm:w-32 sm:h-32 rounded-3xl border-4'} bg-gradient-to-br from-white via-slate-50 to-slate-200 border-slate-300 shadow-[0_12px_28px_rgba(0,0,0,0.6),inset_0_2px_4px_rgba(255,255,255,0.9)] flex items-center justify-center relative cursor-pointer ${
