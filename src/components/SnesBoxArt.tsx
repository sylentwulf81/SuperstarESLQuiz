...
import { LauncherGame } from '../data/launcherGames';
...
export const SnesBoxArt: React.FC<SnesBoxArtProps> = ({
  ...
  // 2. Game's configured image (e.g. /covers/summer.jpg)
  // 3. Fallback default paths based on game id
  const defaultImagePath = game.id === 'mario_party_summer' 
    ? '/covers/summer.jpg'
    : game.id === 'mario_party_winter'
    ? '/covers/winter.jpg'
    : game.cover.backgroundImage;

  const displayImage = customImage || (!imageError ? defaultImagePath : null);
